import { formatWithBuiltin, formatWithBuiltinForTest } from "./builtin.js";
import { formatWithEmbeddedDprint, resetDprintStateForTest } from "./dprint.js";
import { resolveOptions } from "./options.js";
import { stripRangeKeysForTest } from "./semantic.js";
import type {
...
export const __formatBirdConfigBuiltinForTest = async (
  text: string,
  options: FormatBirdConfigOptions = {},
): Promise<BuiltinFormatOutput> => formatWithBuiltinForTest(text, options);

/** Internal-only helper for regression tests. */
export const __stripRangeKeysForTest = (value: unknown): unknown => stripRangeKeysForTest(value);

/** Internal-only helper for deterministic unit tests. */
export const __resetFormatterStateForTest = (): void => {
  resetDprintStateForTest();
};
