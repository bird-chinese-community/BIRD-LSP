import { parseBirdConfig, type BirdDeclaration, type ParseIssue } from "@birdcc/parser";

type ObjectLikeValue = Record<string, unknown> | unknown[];
type ParsedBirdConfig = Awaited<ReturnType<typeof parseBirdConfig>>;
...
export const createSemanticFingerprintFromParsed = (parsed: ParsedBirdConfig): string => {
...
export const createSemanticFingerprint = async (text: string): Promise<string> => {
...
export const stripRangeKeysForTest = (value: unknown): unknown => stripRangeKeys(value);
