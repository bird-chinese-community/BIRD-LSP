...
describe("@birdcc/parser tree-sitter", () => {
  ...
  it("parses common BGP session option statements", async () => {
    ...
    if (protocol?.kind === "protocol") {
      expect(protocol.statements).toMatchObject([
        ...
        { kind: "bgp-option", option: "path-metric", value: true },
  ...
  it("parses BGP default MED option", async () => {
    ...
    if (protocol?.kind === "protocol") {
      ...
      expect(
        protocol.statements.some(
          (item) =>
            item.kind === "other" && /\bdefault bgp_med\b/.test(item.text),
        ),
      ).toBe(false);
    }
  });
  ...
  it("parses BGP allow local preference option", async () => {
    const parsed = await parseBirdConfig(`
      protocol bgp edge {
    ...
    if (protocol?.kind === "protocol") {
      ...
      expect(
        protocol.statements.some(
          (item) =>
            item.kind === "other" && /\ballow bgp_local_pref\b/.test(item.text),
        ),
      ).toBe(false);
    }
  });
  ...
  it("parses BGP min graceful restart time option", async () => {
    const parsed = await parseBirdConfig(`
      protocol bgp edge {
        min graceful restart time 60;
      }
    `);
    ...
    if (protocol?.kind === "protocol") {
      expect(protocol.statements).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            kind: "bgp-option",
            option: "min-graceful-restart-time",
            value: "60",
          }),
        ]),
      );
      expect(
        protocol.statements.some(
          (item) =>
            item.kind === "other" &&
            /\bmin graceful restart time\b/.test(item.text),
        ),
      ).toBe(false);
    }
  });

  it("parses BGP max graceful restart time option", async () => {
    const parsed = await parseBirdConfig(`
      protocol bgp edge {
        max graceful restart time 240;
      }
    `);
    ...
    if (protocol?.kind === "protocol") {
      expect(protocol.statements).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            kind: "bgp-option",
            option: "max-graceful-restart-time",
            value: "240",
          }),
        ]),
      );
      expect(
        protocol.statements.some(
          (item) =>
            item.kind === "other" &&
            /\bmax graceful restart time\b/.test(item.text),
        ),
      ).toBe(false);
    }
  });

  it("parses BGP long lived stale time option", async () => {
    const parsed = await parseBirdConfig(`
      protocol bgp edge {
    ...
    if (protocol?.kind === "protocol") {
      ...
      expect(
        protocol.statements.some(
          (item) =>
            item.kind === "other" &&
            /\blong lived stale time\b/.test(item.text),
        ),
      ).toBe(false);
    }
  });
  ...
  it("parses BGP max long lived stale time option", async () => {
    const parsed = await parseBirdConfig(`
      protocol bgp edge {
        max long lived stale time 7200;
      }
    `);
    ...
    if (protocol?.kind === "protocol") {
      expect(protocol.statements).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            kind: "bgp-option",
            option: "max-long-lived-stale-time",
            value: "7200",
          }),
        ]),
      );
      expect(
        protocol.statements.some(
          (item) =>
            item.kind === "other" &&
            /\bmax long lived stale time\b/.test(item.text),
        ),
      ).toBe(false);
    }
  });
    ...
    const parsed = await parseBirdConfig(`
      protocol bgp edge_peer {
        ipv4 {
          ...
          link local next hop format single;
          mandatory on;
          aigp on;
          aigp originate;
          cost MY_COST;
