Title: BIRD 2.18 User's Guide

URL Source: https://bird.nic.cz/doc/bird-2.18.html
...
Usually, a routing table just chooses a selected route from a list of entries for one network. Optionally, these lists of entries are kept completely sorted (according to preference or some protocol-dependent metric). See [sorted](https://bird.nic.cz/doc/bird-2.18.html#rtable-sorted) table option for details.
...
Every route consists of several attributes (read more about them in the [Route attributes](https://bird.nic.cz/doc/bird-2.18.html#route-attributes) section); the common for all routes are:
...
log "_filename_" [_limit_ "_backup_"] | syslog [name _name_] | stderr | udp _address_ [port _port_] all|{ _list of classes_ }
...
Set global defaults of protocol debugging options. See [debug](https://bird.nic.cz/doc/bird-2.18.html#proto-debug) in the following section. Default: off.

debug channels all|off|{ states|routes|filters|events [, _..._] }
Set global defaults of channel debugging options. See [debug](https://bird.nic.cz/doc/bird-2.18.html#channel-debug) in the channel section. Default: off.

debug tables all|off|{ states|routes|filters|events [, _..._] }
Set global defaults of table debugging options. See [debug](https://bird.nic.cz/doc/bird-2.18.html#table-debug) in the table section. Default: off.
...
Set BIRD's router ID based on an IPv4 address of an interface specified by an interface pattern. See [interface](https://bird.nic.cz/doc/bird-2.18.html#proto-iface) section for detailed description of interface patterns with extended clauses.
...
Define a new routing table. The default routing tables `master4` and `master6` are defined implicitly, other routing tables have to be defined by this option. See the [routing table configuration section](https://bird.nic.cz/doc/bird-2.18.html#rtable-opts) for routing table options.

mpls domain _name_ [ { _option_; [_..._] } ]
Define a new MPLS domain. MPLS domains represent independent label spaces and are responsible for MPLS label management. All MPLS-aware protocols are associated with some MPLS domain. See the [MPLS configuration section](https://bird.nic.cz/doc/bird-2.18.html#mpls-opts) for MPLS domain options.
...
Set table debugging options. Like in [protocol debugging](https://bird.nic.cz/doc/bird-2.18.html#proto-debug), tables are capable of writing trace messages about its work to the log (with category `trace`). For now, this does nothing, but in version 3, it is used. Default: off.
...
This has advantage for [RPKI](https://bird.nic.cz/doc/bird-2.18.html#rpki) (on ROA tables), for [recursive next-hops](https://bird.nic.cz/doc/bird-2.18.html#bgp-gateway) (on IGP tables), and is required for [flowspec validation](https://bird.nic.cz/doc/bird-2.18.html#bgp-validate) (on base IP tables). Another advantage is that interval results (like from `show route in ...` command) are lexicographically sorted. The disadvantage is that trie-enabled routing tables require more memory, which may be an issue especially in multi-table setups. Default: off.

min settle time _time_
Specify a minimum value of the settle time. When a ROA table changes, automatic [RPKI reload](https://bird.nic.cz/doc/bird-2.18.html#proto-rpki-reload) may be triggered, after a short settle time. Minimum settle time is a delay from the last ROA table change to wait for more updates. Default: 1 s.

max settle time _time_
Specify a maximum value of the settle time. When a ROA table changes, automatic [RPKI reload](https://bird.nic.cz/doc/bird-2.18.html#proto-rpki-reload) may be triggered, after a short settle time. Maximum settle time is an upper limit to the settle time from the initial ROA table change even if there are consecutive updates gradually renewing the settle time. Default: 20 s.
...
Note that for proper VRF support it is necessary to use Linux kernel version at least 4.14, older versions have limited VRF implementation. Before Linux kernel 5.0, a socket bound to a port in default VRF collide with others in regular VRFs. In BGP, this can be avoided by using [strict bind](https://bird.nic.cz/doc/bird-2.18.html#bgp-strict-bind) option.

_channel name_ [{_channel config_}]
Every channel must be explicitly stated. See the protocol-specific configuration for the list of supported channel names. See the [channel configuration section](https://bird.nic.cz/doc/bird-2.18.html#channel-opts) for channel definition.
...
A password can be specified as a string or as a sequence of hexadecimal digit pairs ([bytestring](https://bird.nic.cz/doc/bird-2.18.html#type-bytestring)).
...
Import or export filters may depend on route RPKI status (using `roa_check()` or `aspa_check()` operators). In contrast to other filter operators, this status for the same route may change as the content of ROA and ASPA tables changes. When this option is active, BIRD activates automatic reload of affected channels whenever ROA and ASPA tables are updated (after a short settle time). When disabled, route reloads have to be requested manually. The option is ignored if neither `roa_check()` nor `aspa_check()` is used in channel filters. Note that for BGP channels, automatic reload requires [import table](https://bird.nic.cz/doc/bird-2.18.html#bgp-import-table) or [export table](https://bird.nic.cz/doc/bird-2.18.html#bgp-export-table) (for respective direction). Default: on.
...
The policy `static` means no dynamic label allocation is done, and static labels must be set in import filters using the route attribute [mpls_label](https://bird.nic.cz/doc/bird-2.18.html#rta-mpls-label).
...
Dump content of a routing table to a specified file in MRT table dump format. See [MRT protocol](https://bird.nic.cz/doc/bird-2.18.html#mrt) for details.
...
The previous configuration is saved and the user can switch back to it with [configure undo](https://bird.nic.cz/doc/bird-2.18.html#cli-configure-undo) command. The old saved configuration is released (even if the reconfiguration attempt fails due to e.g. a syntax error).
...
reload [in|out] _name_|"_pattern_"|all
...
Meaning of "_format1_", _limit_, and "_format2_" is the same as in the [timeformat](https://bird.nic.cz/doc/bird-2.18.html#opt-timeformat) configuration option. Also, the same `iso ...` shorthands may be used.

down
Shut BIRD down.

graceful restart
Shut BIRD down for graceful restart. See [graceful restart](https://bird.nic.cz/doc/bird-2.18.html#graceful-restart) section for details.
...
Control echoing of log messages to the command-line output. See [log option](https://bird.nic.cz/doc/bird-2.18.html#opt-log) for a list of log classes.
...
_expr_`{ else: |`_set_body_expr_`/:`_statement_`; [... ] }`. The expression after `case` can be of any type that could be a member of a set, while the _set_body_expr_ before `:` can be anything (constants, intervals, expressions) that could be a part of a set literal. One exception is prefix type, which can be used in sets bud not in `case` structure. Multiple commands are allowed without `{}` grouping. If _expr_ matches one of the `:` clauses, statements between it and next `:` statement are executed. If _expr_ matches neither of the `:` clauses, the statements after `else:` are executed.
...
_enum_ mpls_policy
For MPLS-aware protocols, this attribute defines which [MPLS label policy](https://bird.nic.cz/doc/bird-2.18.html#mpls-channel-label-policy) will be used for the route. It can be set in import filters to change it on per-route basis. Valid values are `MPLS_POLICY_NONE` (no label), `MPLS_POLICY_STATIC` (static label), `MPLS_POLICY_PREFIX` (per-prefix label), `MPLS_POLICY_AGGREGATE` (aggregated label), and `MPLS_POLICY_VRF` (per-VRF label). See [MPLS label policy](https://bird.nic.cz/doc/bird-2.18.html#mpls-channel-label-policy) for details.

_int_ mpls_class
When [MPLS label policy](https://bird.nic.cz/doc/bird-2.18.html#mpls-channel-label-policy) is set to `aggregate`, it may be useful to apply more fine-grained aggregation than just one based on next hops. When routes have different value of this attribute, they will not be aggregated under one local label even if they have the same next hops.
...
The Babel protocol support both IPv4 and IPv6 channels; both can be configured simultaneously. It can also be configured with [IPv6 SADR](https://bird.nic.cz/doc/bird-2.18.html#ip-sadr-routes) channel instead of regular IPv6 channel, in such case SADR support is enabled. Babel supports no global configuration options apart from those common to all other protocols, but supports the following per-interface configuration options:
...
These options specify the ToS/DiffServ/Traffic class/Priority of the outgoing Babel packets. See [tx class](https://bird.nic.cz/doc/bird-2.18.html#proto-tx-class) common option for detailed description.
...
next hop prefer native|ipv6
By default, BIRD prefers to advertise IPv4 routes with an IPv4 next hop address, using an IPv6 next hop address only when IPv4 addresses are absent from the interface. When set to `ipv6`, BIRD will advertise IPv4 routes with an IPv6 next hop address even when IPv4 addresses are present on the interface (assuming the option [extended next hop](https://bird.nic.cz/doc/bird-2.18.html#babel-extended-next-hop) is enabled). Default: native.

extended next hop _switch_
Specify whether BIRD should allow IPv4 routes with an IPv6 next hop, as described in [RFC 9229](https://datatracker.ietf.org/doc/rfc9229). Note that when both IPv4 and IPv6 next hops are available, the option [next hop prefer](https://bird.nic.cz/doc/bird-2.18.html#babel-next-hop-prefer) controls which one is advertised. Default: yes.

rtt cost _number_
The RTT-based cost that will be applied to all routes from each neighbour based on the measured RTT to that neighbour. If this value is set, timestamps will be included in generated Babel Hello and IHU messages, and (if the neighbours also have timestamps enabled), the RTT to each neighbour will be computed. An additional cost is added to a neighbour if its RTT is above the [rtt min](https://bird.nic.cz/doc/bird-2.18.html#babel-rtt-min) value configured on the interface. The added cost scales linearly from 0 up to the RTT cost configured in this option; the full cost is applied if the neighbour RTT reaches the RTT configured in the [rtt max](https://bird.nic.cz/doc/bird-2.18.html#babel-rtt-max) option (and for all RTTs above this value). Default: 0 (disabled), except for tunnel interfaces, where it is 96.
...
password "_text_"
Specifies a password used for authentication. See [password](https://bird.nic.cz/doc/bird-2.18.html#proto-pass) common option for detailed description. Note that password option `algorithm` is not available in BFD protocol. The algorithm is selected by `authentication` option for all passwords.
...
authentication none|md5|ao
Selects authentication method to be used. `none` means that the BGP session is not authenticated at all. `md5` means that the TCP MD5 authentication of BGP sessions ([RFC 2385](https://datatracker.ietf.org/doc/rfc2385)) is used, in that case the option [password](https://bird.nic.cz/doc/bird-2.18.html#bgp-password) is used to specify the (single) password. Finally, `ao` means to use TCP Authentication Option (TCP-AO, [RFC 5925](https://datatracker.ietf.org/doc/rfc5925)), allowing multiple keys and different cryptographic algorithms. These are specified using the option [keys](https://bird.nic.cz/doc/bird-2.18.html#bgp-keys). Note that TCP-AO authentication is not supported on dynamic BGP sessions. Default: none.
...
Of course, TCP-AO key contains a shared secret key. It is specified by the option `secret` as a text string or as a sequence of hexadecimal digit pairs ([bytestring](https://bird.nic.cz/doc/bird-2.18.html#type-bytestring)).
...
For IPv6 routes, the Next Hop attribute may contain both a global IP address and a link-local IP address. For IBGP sessions, the global IP address is resolved ([gateway recursive](https://bird.nic.cz/doc/bird-2.18.html#bgp-gateway)) through an IGP routing table ([igp table](https://bird.nic.cz/doc/bird-2.18.html#bgp-igp-table)) to get an immediate next hop. If the resulting IGP route is a direct route (i.e., the next hop is a direct neighbor), then the link-local IP address from the Next Hop attribute is used as the immediate next hop. This option changes it to prefer global or link-local IP address, respectively. Note that even with this option set to global, route may end with a link-local immediate next hop when the IGP route has one.
...
Note that currently the import table breaks routes with recursive nexthops (e.g. ones from IBGP, see [gateway recursive](https://bird.nic.cz/doc/bird-2.18.html#bgp-gateway)), they are not properly updated after next hop change. For the same reason, it also breaks re-evaluation of flowspec routes with [flowspec validation](https://bird.nic.cz/doc/bird-2.18.html#bgp-validate) option enabled on flowspec channels.
...
secondary _switch_
Usually, if an export filter rejects a selected route, no other route is propagated for that network. This option allows to try the next route in order until one that is accepted is found or all routes for that network are rejected. This can be used for route servers that need to propagate different tables to each client but do not want to have these tables explicitly (to conserve memory). This option requires that the connected routing table is [sorted](https://bird.nic.cz/doc/bird-2.18.html#dsc-table-sorted). Default: off.

validate _switch_
Apply flowspec validation procedure as described in [RFC 8955](https://datatracker.ietf.org/doc/rfc8955) section 6 and [RFC 9117](https://datatracker.ietf.org/doc/rfc9117). The Validation procedure enforces that only routers in the forwarding path for a network can originate flowspec rules for that network. The validation procedure should be used for EBGP to prevent injection of malicious flowspec rules from outside, but it should also be used for IBGP to ensure that selected flowspec rules are consistent with selected IP routes. The validation procedure uses an IP routing table ([base table](https://bird.nic.cz/doc/bird-2.18.html#bgp-base-table), see below) against which flowspec rules are validated. This option is limited to flowspec channels. Default: off (for compatibility reasons).

Note that currently the flowspec validation does not work reliably together with [import table](https://bird.nic.cz/doc/bird-2.18.html#bgp-import-table) option enabled on flowspec channels.
...
cost _number_
When BGP [gateway mode](https://bird.nic.cz/doc/bird-2.18.html#bgp-gateway) is `recursive` (mainly multihop IBGP sessions), then the distance to BGP next hop is based on underlying IGP metric. This option specifies the distance to BGP next hop for BGP sessions in direct gateway mode (mainly direct EBGP sessions).

graceful restart _switch_
Although BGP graceful restart is configured mainly by protocol-wide [options](https://bird.nic.cz/doc/bird-2.18.html#bgp-graceful-restart), it is possible to configure restarting role per AFI/SAFI pair by this channel option. The option is ignored if graceful restart is disabled by protocol-wide option. Default: off in aware mode, on in full mode.

long lived graceful restart _switch_
BGP long-lived graceful restart is configured mainly by protocol-wide [options](https://bird.nic.cz/doc/bird-2.18.html#bgp-long-lived-graceful-restart), but the restarting role can be set per AFI/SAFI pair by this channel option. The option is ignored if long-lived graceful restart is disabled by protocol-wide option. Default: off in aware mode, on in full mode.

long lived stale time _number_
Like previous graceful restart channel options, this option allows to set [long lived stale time](https://bird.nic.cz/doc/bird-2.18.html#bgp-long-lived-stale-time) per AFI/SAFI pair instead of per protocol. Default: set by protocol-wide option.

min long lived stale time _number_
Like previous graceful restart channel options, this option allows to set [min long lived stale time](https://bird.nic.cz/doc/bird-2.18.html#bgp-min-long-lived-stale-time) per AFI/SAFI pair instead of per protocol. Default: set by protocol-wide option.

max long lived stale time _number_
Like previous graceful restart channel options, this option allows to set [max long lived stale time](https://bird.nic.cz/doc/bird-2.18.html#bgp-max-long-lived-stale-time) per AFI/SAFI pair instead of per protocol. Default: set by protocol-wide option.
...
int bgp_otc [O]
This attribute is defined in [RFC 9234](https://datatracker.ietf.org/doc/rfc9234). OTC is a flag that marks routes that should be sent only to customers. If [local role](https://bird.nic.cz/doc/bird-2.18.html#bgp-local-role) is configured it set automatically.
...
The implementation supports monitoring protocol state changes, pre-policy routes (in [BGP import tables](https://bird.nic.cz/doc/bird-2.18.html#bgp-import-table)) and post-policy routes (in regular routing tables). All BGP protocols are monitored automatically.
...
The Kernel protocol supports both IPv4 and IPv6 channels; only one channel can be configured in each protocol instance. On Linux, it also supports [IPv6 SADR](https://bird.nic.cz/doc/bird-2.18.html#ip-sadr-routes) and [MPLS](https://bird.nic.cz/doc/bird-2.18.html#mpls-routes) channels.
...
In BGP/MPLS IP VPNs, route distribution is controlled by Route Targets (RT). VRFs are associated with one or more RTs. Routes are also associated with one or more RTs, which are encoded as route target extended communities in [bgp_ext_community](https://bird.nic.cz/doc/bird-2.18.html#rta-bgp-ext-community). A route is then imported into each VRF that shares an associated Route Target. The L3VPN protocol implements this mechanism through mandatory `import target` and `export target` protocol options.
...
Defines that the specified interfaces belong to the area being defined. See [interface](https://bird.nic.cz/doc/bird-2.18.html#proto-iface) common option for detailed description. In OSPFv2, extended interface clauses are used, because each network prefix is handled as a separate virtual interface.
...
tx class|dscp|priority _number_
These options specify the ToS/DiffServ/Traffic class/Priority of the outgoing OSPF packets. See [tx class](https://bird.nic.cz/doc/bird-2.18.html#proto-tx-class) common option for detailed description.
...
password "_text_"
Specifies a password used for authentication. See [password](https://bird.nic.cz/doc/bird-2.18.html#proto-pass) common option for detailed description.
...
interface _pattern_ [, _..._] { _options_ }
Interface definitions specify a set of interfaces on which the protocol is activated and contain interface specific options. See [interface](https://bird.nic.cz/doc/bird-2.18.html#proto-iface) common options for detailed description.
...
This option specifies the time (in seconds) how long (since the receipt of RA) hosts may use the router as a default router. 0 means do not use as a default router. For `sensitive` option, see [trigger](https://bird.nic.cz/doc/bird-2.18.html#radv-trigger). Default: 3 \* `max ra interval`, `sensitive` yes.
...
This option specifies the default value of advertised lifetime for specific routes; i.e., the time (in seconds) for how long (since the receipt of RA) hosts should consider these routes valid. A special value 0xffffffff represents infinity. The lifetime can be overriden on a per route basis by the [ra_lifetime](https://bird.nic.cz/doc/bird-2.18.html#rta-ra-lifetime) route attribute. Default: 3 \* `max ra interval`, `sensitive` no.

For the `sensitive` option, see [trigger](https://bird.nic.cz/doc/bird-2.18.html#radv-trigger). If `sensitive` is enabled, even the routes with the `ra_lifetime` attribute become sensitive to the trigger.

route preference low|medium|high
This option specifies the default value of advertised route preference for specific routes. The value can be overriden on a per route basis by the [ra_preference](https://bird.nic.cz/doc/bird-2.18.html#rta-ra-preference) route attribute. Default: medium.
...
This option specifies the time (in seconds) how long (after the receipt of RA) the prefix information is valid, i.e., autoconfigured IP addresses can be assigned and hosts with that IP addresses are considered directly reachable. 0 means the prefix is no longer valid. For `sensitive` option, see [trigger](https://bird.nic.cz/doc/bird-2.18.html#radv-trigger). Default: 86400 (1 day), `sensitive` no.

preferred lifetime _expr_ [sensitive _switch_]
This option specifies the time (in seconds) how long (after the receipt of RA) IP addresses generated from the prefix using stateless autoconfiguration remain preferred. For `sensitive` option, see [trigger](https://bird.nic.cz/doc/bird-2.18.html#radv-trigger). Default: 14400 (4 hours), `sensitive` no.
...
enum ra*preference
The preference of the route. The value can be \_RA_PREF_LOW*, _RA_PREF_MEDIUM_ or _RA_PREF_HIGH_. If the attribute is not set, the [route preference](https://bird.nic.cz/doc/bird-2.18.html#radv-iface-route-preference) option is used.

int ra_lifetime
The advertised lifetime of the route, in seconds. The special value of 0xffffffff represents infinity. If the attribute is not set, the [route lifetime](https://bird.nic.cz/doc/bird-2.18.html#radv-iface-route-lifetime) option is used.
...
tx class|dscp|priority _number_
These options specify the ToS/DiffServ/Traffic class/Priority of the outgoing RIP packets. See [tx class](https://bird.nic.cz/doc/bird-2.18.html#proto-tx-class) common option for detailed description.
...
There exist several types of routes; keep in mind that _prefix_ syntax is [dependent on network type](https://bird.nic.cz/doc/bird-2.18.html#type-prefix).

route _prefix_ [mpls _number_] via _ip_|_"interface"_ [_per-nexthop options_] [via ...]
Regular routes may bear one or more [next hops](https://bird.nic.cz/doc/bird-2.18.html#route-next-hop). Every next hop is preceded by `via` and configured as shown.
